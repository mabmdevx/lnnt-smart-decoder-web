...
if(isset($_POST['checkloginpostbk']) && ($_POST['checkloginpostbk']==1) )
{
	...
	if(isset($errorMsg) && strlen($errorMsg)<=0) // If No Error - Start
	{
		...
		$usersQry="select sysuser_id,sysuser_login,sysuser_password,sysuser_role,sysuser_email from sysuser where sysuser_login='".$username."' AND is_deleted = 'no' AND sysuser_status='active' ";
